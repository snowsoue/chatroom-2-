...
const onlineUsers = new Map(); // socket.id -> { username, avatarColor, avatarUrl, avatarPosX, avatarPosY, channel, ip }
...
io.on('connection', (socket) => {
  ...
  socket.on('join', ({ username, avatarColor, avatarUrl, avatarPosX, avatarPosY, channel }) => {
    ...
    avatarPosX = clampPercent(avatarPosX);
    avatarPosY = clampPercent(avatarPosY);
    ...
    onlineUsers.set(socket.id, { username, avatarColor, avatarUrl, avatarPosX, avatarPosY, channel, ip });
    socket.join(channel);
  ...
  socket.on('message', ({ text, imageUrl }) => {
    ...
    const stmt = db.prepare(
      `INSERT INTO messages (channel_id, username, avatar_color, avatar_url, avatar_pos_x, avatar_pos_y, text, image_url, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    );
    const info = stmt.run(
      user.channel, user.username, user.avatarColor, user.avatarUrl,
      user.avatarPosX, user.avatarPosY, text, imageUrl, created_at
    );
    ...
    const message = {
      ...
      avatar_pos_x: user.avatarPosX,
      avatar_pos_y: user.avatarPosY,
      text,
      image_url: imageUrl,
      reactions: [],
      created_at
