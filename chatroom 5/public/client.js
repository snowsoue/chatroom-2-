(() => {
  ...
  function switchChannel(channelId) {
    ...
    channelNameEl.textContent = ch ? ch.name : channelId;
    channelTopicEl.textContent = ch ? ch.topic : '';
    messageInput.placeholder = `Message #${ch ? ch.name : channelId}`;
  ...
  function renderMessage(m, prepend) {
    ...
    if (grouped) {
      gutter.innerHTML = `<span class="hover-timestamp">${formatTime(m.created_at)}</span>`;
    } else {
    ...
    if (!grouped) {
      ...
      header.innerHTML = `<span class="message-author">${escapeHtml(m.username)}</span><span class="message-timestamp" title="${formatFullTime(m.created_at)}">${formatTime(m.created_at)}</span>`;
      body.appendChild(header);
    }

    const content = document.createElement('div');
    content.className = 'message-content' + (isEmojiOnlyMessage(m.text) && !m.image_url ? ' jumbo' : '');
    let html = '';
    if (m.text) html += `<div class="message-text">${renderTextContent(m.text)}</div>`;
