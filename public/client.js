(() => {
  ...
  const avatarRepositionBtn = document.getElementById('avatar-reposition-btn');
  const repositionModal = document.getElementById('reposition-modal');
  const repositionDragArea = document.getElementById('reposition-drag-area');
  const repositionSaveBtn = document.getElementById('reposition-save-btn');
  const repositionCancelBtn = document.getElementById('reposition-cancel-btn');
  ...
  // posX/posY are percentages (0-100) for where the image is focused within the circle —
  // same idea as CSS background-position, which is exactly what this sets.
  function applyAvatarStyle(el, color, url, label, posX, posY) {
    el.style.background = url ? `url(${url})` : color;
    el.style.backgroundPosition = url ? `${posX ?? 50}% ${posY ?? 50}%` : '';
    el.textContent = url ? '' : initials(label);
  }

  function refreshPreview() {
    applyAvatarStyle(avatarPreview, state.avatarColor, state.avatarUrl, usernameInput.value, state.avatarPosX, state.avatarPosY);
  }

  function updateRepositionButtonVisibility() {
    avatarRepositionBtn.classList.toggle('hidden', !state.avatarUrl);
  }
  ...
  document.getElementById('avatar-color-btn').addEventListener('click', () => {
    ...
    state.avatarPosY = 50;
    refreshPreview();
    updateRepositionButtonVisibility();
  });
  ...
  avatarFile.addEventListener('change', () => {
    ...
    reader.onload = () => {
      ...
      state.avatarPosX = 50; // fresh upload starts centered — nudge with "Adjust photo position" if needed
      state.avatarPosY = 50;
      refreshPreview();
      updateRepositionButtonVisibility();
    };
  ...
  function doJoin() {
    ...
    localStorage.setItem('chat_avatar_pos_x', String(state.avatarPosX));
    localStorage.setItem('chat_avatar_pos_y', String(state.avatarPosY));

    loginScreen.classList.add('hidden');
    app.classList.remove('hidden');

    applyAvatarStyle(myAvatarEl, state.avatarColor, state.avatarUrl, state.username, state.avatarPosX, state.avatarPosY);
  ...
  (function restore() {
    ...
    const savedPosX = parseFloat(localStorage.getItem('chat_avatar_pos_x'));
    const savedPosY = parseFloat(localStorage.getItem('chat_avatar_pos_y'));
    if (savedName) usernameInput.value = savedName;
    if (savedColor) state.avatarColor = savedColor;
    if (savedUrl) state.avatarUrl = savedUrl;
    if (Number.isFinite(savedPosX)) state.avatarPosX = clamp(savedPosX, 0, 100);
    if (Number.isFinite(savedPosY)) state.avatarPosY = clamp(savedPosY, 0, 100);
    refreshPreview();
    updateRepositionButtonVisibility();
  })();
  ...
  // ---------- Avatar reposition (drag to choose focus point) ----------
  // Working copy of the position while the modal is open — only committed to state on Save,
  // so Cancel can discard it without touching the avatar the person already has.
  let repositionTempX = 50;
  let repositionTempY = 50;
  let repositionDrag = null; // { startClientX, startClientY, startX, startY } while actively dragging

  function setDragAreaPosition(posX, posY) {
    repositionDragArea.style.backgroundPosition = `${posX}% ${posY}%`;
  }

  function openRepositionModal() {
    if (!state.avatarUrl) return;
    repositionTempX = state.avatarPosX;
    repositionTempY = state.avatarPosY;
    repositionDragArea.style.backgroundImage = `url(${state.avatarUrl})`;
    setDragAreaPosition(repositionTempX, repositionTempY);
    repositionModal.classList.remove('hidden');
  }

  function closeRepositionModal() {
    repositionModal.classList.add('hidden');
    repositionDrag = null;
  }

  avatarRepositionBtn.addEventListener('click', openRepositionModal);
  repositionCancelBtn.addEventListener('click', closeRepositionModal);

  repositionSaveBtn.addEventListener('click', () => {
    state.avatarPosX = repositionTempX;
    state.avatarPosY = repositionTempY;
    localStorage.setItem('chat_avatar_pos_x', String(state.avatarPosX));
    localStorage.setItem('chat_avatar_pos_y', String(state.avatarPosY));
    refreshPreview();
    ...
    // Already in the chat? Re-announce so everyone's view (and future messages) picks up the new framing.
    // Past messages keep whatever framing was current when they were sent — same as a profile photo change
    // on Instagram not rewriting old posts.
  ...
  repositionDragArea.addEventListener('pointerdown', (e) => {
    repositionDrag = {
      startClientX: e.clientX,
      startClientY: e.clientY,
      startX: repositionTempX,
      startY: repositionTempY
    };
    repositionDragArea.setPointerCapture(e.pointerId);
  });

  repositionDragArea.addEventListener('pointermove', (e) => {
    if (!repositionDrag) return;
    const rect = repositionDragArea.getBoundingClientRect();
    const dxPercent = ((e.clientX - repositionDrag.startClientX) / rect.width) * 100;
    const dyPercent = ((e.clientY - repositionDrag.startClientY) / rect.height) * 100;
    // Dragging right should reveal more of the image's left side, so position moves opposite to the drag.
    repositionTempX = clamp(repositionDrag.startX - dxPercent, 0, 100);
    repositionTempY = clamp(repositionDrag.startY - dyPercent, 0, 100);
    setDragAreaPosition(repositionTempX, repositionTempY);
  });

  ['pointerup', 'pointercancel'].forEach(evt => {
    repositionDragArea.addEventListener(evt, () => { repositionDrag = null; });
  });
  ...
  async function loadChannels() {
    try {
      ...
      console.log(`Loaded ${state.channels.length} channel(s):`, state.channels.map(c => c.id));
      renderChannelList();
      switchChannel(state.channel);
    } catch (err) {
      console.error('Failed to load channels:', err);
      channelListEl.innerHTML = '<div class="fetch-error">Couldn\'t load channels — see the browser console for details.</div>';
    }
  }
  ...
  async function loadHistory(before) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Server returned ${res.status} ${res.statusText}`);
      rows = await res.json();
      console.log(`Loaded ${rows.length} message(s) for #${state.channel}${before ? ' (older page)' : ''}`);
    } catch (err) {
      console.error('Failed to load message history:', err);
      if (!before) {
        messagesInnerEl.innerHTML = '<div class="fetch-error">Couldn\'t load messages — see the browser console for details.</div>';
      }
      return;
    }
  ...
  function renderMessage(m, prepend) {
    ...
    if (!isMine) {
      ...
      if (!grouped) {
        ...
        applyAvatarStyle(avatar, m.avatar_color, m.avatar_url, m.username, m.avatar_pos_x, m.avatar_pos_y);
  ...
  function renderMembers(users) {
    ...
    users.forEach(u => {
      ...
      applyAvatarStyle(avatar, u.avatarColor, u.avatarUrl, u.username, u.avatarPosX, u.avatarPosY);
